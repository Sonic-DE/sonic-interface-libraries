...
T.ScrollBar {
    ...
    leftPadding: scrollbarSvg.hasElement(`${handle.usedPrefix}-hint-left-inset`) ? handle.inset.left/* + (!mirrored && separator.visible ? separator.implicitWidth : 0)*/ : horizontalPadding
    ...
    contentItem: PlasmaCore.FrameSvgItem {
        ...
        implicitHeight: Math.max(scrollbarSvg.elementSize("hint-scrollbar-size").height, fixedMargins.top + fixedMargins.bottom)
        colorGroup: PlasmaCore.ColorScope.colorGroup
        prefix: controlRoot.active && controlRoot.interactive && controlRoot.enabled ? "mouseover-slider" : "slider"

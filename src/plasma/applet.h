...
class PLASMA_EXPORT Applet : public QObject
{
    ...
    Q_PROPERTY(bool configurationRequired READ configurationRequired WRITE setConfigurationRequired NOTIFY
                   configurationRequiredChanged) // TODO KF6: having just a reason property and required would be string not empty? Uglier from c++ pov but more
                                                 // straight forward from qml pov
...
private:
     ...
     * \a parent a QObject parent; you probably want to pass in 0
     *
     * \a args a list of strings containing two entries: the service id
     *      and the applet id
     *
     * \since 4.3
     */

...
KConfig getConfigForTheme(const QString &theme)
{
    ...
    qWarning() << Q_FUNC_INFO << configPath;
    ...
    qWarning() << Q_FUNC_INFO << metadataPath;
    return KConfig(metadataPath, KConfig::SimpleConfig);
}

KPluginMetaData getMetaDataForTheme(const QString &theme)
{
...
void ThemePrivate::setThemeName(const QString &tempThemeName, bool writeSettings, bool emitChanged)
{
    ...
    if (realTheme) {
        KPluginMetaData data = getMetaDataForTheme(theme);
        if (!data.isValid()) {
            data = getMetaDataForTheme(QStringLiteral("default"));
            if (!data.isValid()) {
                return;
            }
    ...
    if (realTheme) {
        pluginMetaData = getMetaDataForTheme(theme);
        KConfig metadata = getConfigForTheme(theme);
        ...
        while (!fallback.isEmpty() && !fallbackThemes.contains(fallback)) {
            ...
            KConfig metadata = getConfigForTheme(fallback);
            KConfigGroup cg(&metadata, "Settings");

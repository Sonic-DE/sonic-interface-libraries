...
public:
    ...
    bool mainItemContainsPosition(const QPointF &point);
    QPointF positionAdjustedForMainItem(const QPointF &point);
...
bool DialogPrivate::mainItemContainsPosition(const QPointF &point)
{
    if (!mainItem) {
        return false;
    }
...
QPointF DialogPrivate::positionAdjustedForMainItem(const QPointF &point)
{
    if (!mainItem) {
        return point;
    }
    ...
    return QPointF(qBound(itemRect.left() + 2, point.x(), itemRect.right() - 2),
                   qBound(itemRect.top() + 2, point.y(), itemRect.bottom() - 2));
}

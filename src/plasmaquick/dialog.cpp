    ...
    if (visible) {
        ...
        if (q->location() == Plasma::Types::FullScreen) {
            ...
            if (type != Dialog::OnScreenDisplay) {
                PlasmaWaylandShellIntegration::get(q)->setPosition(q->position());
            }
        }
    }
...
void DialogPrivate::applyType()
{
    ...
    if (q->flags() & Qt::WindowStaysOnTopHint) {
        ...
        if (type != Dialog::AppletPopup && type != Dialog::Tooltip) {
            type = Dialog::Dock;
            PlasmaWaylandShellIntegration::get(q)->setPanelBehavior(QtWayland::org_kde_plasma_surface::panel_behavior_windows_go_below);
        } else {
            PlasmaWaylandShellIntegration::get(q)->setPanelBehavior(QtWayland::org_kde_plasma_surface::panel_behavior_always_visible);
        }
    }
    switch (type) {
    case Dialog::Dock:
        PlasmaWaylandShellIntegration::get(q)->setRole(QtWayland::org_kde_plasma_surface::role_panel);
        break;
    case Dialog::Tooltip:
        PlasmaWaylandShellIntegration::get(q)->setRole(QtWayland::org_kde_plasma_surface::role_tooltip);
        break;
    case Dialog::Notification:
        PlasmaWaylandShellIntegration::get(q)->setRole(QtWayland::org_kde_plasma_surface::role_notification);
        break;
    case Dialog::OnScreenDisplay:
        PlasmaWaylandShellIntegration::get(q)->setRole(QtWayland::org_kde_plasma_surface::role_onscreendisplay);
        break;
    case Dialog::CriticalNotification:
        PlasmaWaylandShellIntegration::get(q)->setRole(QtWayland::org_kde_plasma_surface::role_criticalnotification);
        break;
    case Dialog::Normal:
        PlasmaWaylandShellIntegration::get(q)->setRole(QtWayland::org_kde_plasma_surface::role_normal);
        break;
    case Dialog::AppletPopup:
        PlasmaWaylandShellIntegration::get(q)->setRole(QtWayland::org_kde_plasma_surface::role_appletpopup);
        break;
    default:
        break;
    }
    ...
    PlasmaWaylandShellIntegration::get(q)->setTakesFocus(q->flags().testFlag(Qt::WindowDoesNotAcceptFocus));
}
...
bool Dialog::event(QEvent *event)
{
    if (event->type() == QEvent::Expose) {
        ...
        PlasmaWaylandShellIntegration::get(this)->setPosition(me->pos());
    }

...
void ContainmentItem::mimeTypeRetrieved(KIO::Job *job, const QString &mimetype)
{
    ...
    QPoint sceneDropPoint = m_dropMenu->screenDropPoint();
    if (window()) {
    ...
    if (mimetype.isEmpty() && appletList.isEmpty()) {
        ...
        qCDebug(LOG_PLASMAQUICK) << "Received a suitable dropEvent at global" << m_dropMenu->screenDropPoint() << "local" << sceneDropPoint;

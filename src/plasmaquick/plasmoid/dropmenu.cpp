...
DropMenu::DropMenu(KIO::DropJob *dropJob, const QPoint &screenDropPoint, ContainmentItem *parent)
    : QObject(parent)
    , m_screenDropPoint(screenDropPoint)
    , m_dropJob(dropJob)
{
    if (!dropJob) {
...
QPoint DropMenu::screenDropPoint() const
{
    return m_screenDropPoint;
}
...
void DropMenu::show()
{
    ...
    if (m_dropJob) {
        ...
        m_dropJob->showMenu(m_screenDropPoint);
        ...
        m_menu->popup(m_screenDropPoint);
    }
}

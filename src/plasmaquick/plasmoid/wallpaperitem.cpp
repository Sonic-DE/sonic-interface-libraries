...
QList<KPluginMetaData> WallpaperItem::listWallpaperMetadataForMimetype(const QString &mimetype, const QString &formFactor)
{
    auto filter = [&mimetype, &formFactor](const KPluginMetaData &md) -> bool {
        if (!formFactor.isEmpty() && !md.value(QStringView(u"X-Plasma-FormFactors")).contains(formFactor)) {
            return false;
        }
...
WallpaperItem *WallpaperItem::loadWallpaper(ContainmentItem *containmentItem)
{
    ...
    const QString rootPath = pkg.metadata().value(QStringView(u"X-Plasma-RootPath"));
    if (!rootPath.isEmpty()) {
...
{
    return m_pkg.metadata().value(QStringView(u"X-Plasma-DropMimeTypes"), QStringList()).contains(mimetype);
}

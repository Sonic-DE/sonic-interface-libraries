...
void PopupPlasmaWindow::setPopupDirection(Qt::Edge popupDirection)
{
    ...
    borders.setFlag(oppositeEdge(popupDirection), false);
    setBorders(borders);
    queuePositionUpdate();
...
void PlasmaQuick::PopupPlasmaWindow::updatePosition()
{
    ...
    if (!m_visualParent) {
        qWarning() << "Exposed with no visual parent. Window positioning broken.";
        return;
    }
...
{
    PlasmaWaylandShellIntegration::get(this)->setPosition(position);
}

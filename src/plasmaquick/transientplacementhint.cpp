...
QRect TransientPlacementHelper::popupRect(QWindow *w, const TransientPlacementHint &placement)
{
    ...
    if (w->transientParent()) {
        ...
        qDebug() << "have a transient parent, mapping" << placement.parentAnchorArea() << " to " << globalParentAnchorRect;
    } else {
        qDebug() << "no transient parent, using " << globalParentAnchorRect;
    }
